...
Sagecare is a WordPress starter theme inspired by (and large chunks of it just plain stolen from) a few great sources that deserve credit:
...
* Rename `sagecare` to your project name.
...
* Do case-sensitive search/replace for the following to match the slug chosen for your theme directory:
	* **SAGECARE** (e.g., PROJECT_NAME)
	* **Chancelight\\Sagecare** (e.g., Client_Name\\Project_Name)
	* **sagecare-** (e.g., project-name-)
	* **sagecare_** (e.g., project_name_)
	* **sagecare** (e.g., project-name)
	* **Sagecare** (e.g., Project Name)
